...
MODEL_CACHE_PATH = (
    os.getenv("MODEL_CACHE_PATH", "/runpod-volume/models")
    if PROD
    else MODEL_CACHE_PATH_DEV
)
# AWS config
...
AWS_OUTPUT_BUCKET_FOLDER = os.getenv("AWS_OUTPUT_BUCKET_FOLDER", "")
AWS_REGION_DEFAULT = "us-east-1"
AWS_REGION = os.getenv("AWS_REGION", AWS_REGION_DEFAULT)
ENABLE_S3_ACCELERATION = os.getenv("ENABLE_S3_ACCELERATION", "FALSE") == "TRUE"
...
def start_comfyui():
    ...
    try:
        print(
            "Checking if ComfyUI instance is already running before starting new ComfyUI instance"
        )
    ...
    try:
        ...
        stdout_thread = threading.Thread(
            target=stream_output, args=(process.stdout, "ComfyUI"), daemon=True
        )
...
def wait_for_comfyui():
    ...
    while True:
        try:
            ...
            if response.status_code != 200:
                raise RuntimeError(
                    f"ERROR: Got error {response.status_code} from ComfyUI"
                )
            return
        except requests.exceptions.RequestException:
...
def setup_comfyui_session():
    ...
    retries = requests.adapters.Retry(
        total=10, backoff_factor=0.1, status_forcelist=[502, 503, 504]
    )
...
def queue_prompt(user_prompt, workflow_data):
    ...
    try:
        ...
        request_data = {"prompt": workflow_data, "client_id": COMFYUI_CLIENT_ID}
...
def get_s3_client():
    ...
    if s3_client:
        ...
        config = boto3.session.Config(
            s3={"use_accelerate_endpoint": ENABLE_S3_ACCELERATION},
        ...
        print(
            f"Connected to S3 {'with' if ENABLE_S3_ACCELERATION else 'without'} transfer acceleration"
        )
    return s3_client
...
def upload_image(job_id, folder_name=None):
    """
    ...
    """
    if AWS_OUTPUT_BUCKET_NAME and AWS_OUTPUT_BUCKET_NAME != "":
        ...
        try:
            ...
            if AWS_OUTPUT_BUCKET_FOLDER and AWS_OUTPUT_BUCKET_FOLDER != "":
                folder_name = AWS_OUTPUT_BUCKET_FOLDER.strip(
                    "/"
            ...
            print(f"Uploading image file to S3 path: {s3_key}")
            ...
            # Use accelerate endpoint URL format if enabled
            if ENABLE_S3_ACCELERATION:
                url = f"https://{AWS_OUTPUT_BUCKET_NAME}.s3-accelerate.amazonaws.com/{s3_key}"
            else:
                url = f"https://{AWS_OUTPUT_BUCKET_NAME}.s3.{AWS_REGION}.amazonaws.com/{s3_key}"
            ...
            print(
                "Check if AWS creds are added in env variables, they might be missing"
            )
            raise
        except Exception as e:
...
def download_input_file(input_filename):
    if not AWS_INPUT_BUCKET_NAME or AWS_INPUT_BUCKET_NAME == "":
        raise ValueError("ERROR: AWS_INPUT_BUCKET_NAME not configured")
    ...
    # Local path where we'll save the downloaded file
    local_path = f"{input_dir}/{input_filename}"

    print(
        f"Downloading input file {input_filename} from S3 bucket {AWS_INPUT_BUCKET_NAME}"
    )
    try:
        s3_client = get_s3_client()
        s3_client.download_file(AWS_INPUT_BUCKET_NAME, input_filename, local_path)
        print(f"Successfully downloaded input file to {local_path}")
        return local_path

    except botocore.exceptions.ClientError as e:
        if e.response["Error"]["Code"] == "404":
            raise FileNotFoundError(
                f"ERROR: Input file {input_filename} not found in bucket {AWS_INPUT_BUCKET_NAME}"
            )
        else:
            print(f"ERROR: Failed to download input file: {str(e)}")
            raise
    except Exception as e:
        print(f"ERROR: Unexpected error downloading input file: {str(e)}")
        raise
                ...
                try:
                    ...
                    if type in ["executing", "progress"]:
                        if "node" in data and data["node"]:
                            ...
                            job_progress_percentage = float(
                                100 * (float(len(nodes_seen)) - 1) / float(total_nodes)
                            )
                            ...
                            job_progress_percentage = int(
                                math.ceil(job_progress_percentage)
                            )
                            print(
                                f"Job {job_id} with prompt {prompt_id} is {job_progress_percentage}% done"
                            )
                        ...
                        print(
                            f"ComfyUI generation for job {job_id} completed successfully"
                        )
                        print(
                            f"Job {job_id} with prompt {prompt_id} is {job_progress_percentage}% done"
                        )
...
async def process_job(
    user_prompt, workflow, aspect_ratio, input_filename, job_id, job_event
):
    """
    ...
    try:
        ...
        if input_filename:
            local_input_path = download_input_file(input_filename)
        ...
        prompt_id = queue_prompt(user_prompt, workflow_data)
        ...
        result = None
        if ENABLE_S3_UPLOAD:
            image_url = upload_image(job_id)
            result = image_url
        else:
            base64_image_data = get_base64_image(job_id)
        ...
        raise TimeoutError(
            f"ERROR: ComfyUI prompt request timed out after {COMFYUI_JOB_TIMEOUT_SEC} seconds"
        )
    finally:
        ...
        if local_input_path and os.path.exists(local_input_path):
            try:
                ...
                print(f"Cleaned up input file: {local_input_path}")
            except Exception as e:
                print(f"Warning: Failed to cleanup input file: {str(e)}")
...
async def handler(event):
    ...
    try:
        ...
        if "prompt" not in event["input"]:
            raise RuntimeError(
                "ERROR: missing 'input.prompt' field in runpod handler request"
            )
        ...
        input_filename = None
        if "input_filename" in event["input"]:
            input_filename = event["input"]["input_filename"]

        return await process_job(
            prompt, workflow, aspect_ratio, input_filename, job_id, event
        )
